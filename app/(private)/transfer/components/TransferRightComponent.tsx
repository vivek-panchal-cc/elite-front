...
const content: {
    ...
    0: {
        ...
        description: "How much would you like to transfer from your rewards account to SIMply?"
    },
    1: {
        image: images.vapeJucceAc,
        title: "Transfer to Your Vape Jucce Account",
        description: "How much would you like to transfer from your rewards account to Vape Jucce?"
    },
    2: {
        image: images.paypal,
        title: "Transfer to Your paypal Account",
        description: "How much would you like to transfer from your rewards account to Paypal?"
    }
}


const TransferRightComponent = ({ isMobile, activeIndex = 0, setIsOpen }: { isMobile: boolean, activeIndex: number, setIsOpen: React.Dispatch<React.SetStateAction<boolean>> }) => {
  return (
    <div className={`${isMobile ? "w-[95%] mx-auto" : "w-3/4"}`}>
      ...
      >
        <div className="flex flex-wrap lg:flex-nowrap gap-5 md:gap-14 justify-center text-center lg:text-left">
            <div>
                <Image className="h-[80px] md:h-auto w-auto" src={content[activeIndex].image} alt="transfer-img"/>
            </div>
            <div className="flex flex-col gap-2  md:gap-4">
                <h4 className="text-[18px] md:text-[29px] font-bold leading-[30px]">{content[activeIndex].title}</h4>
